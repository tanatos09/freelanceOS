...
class APITester:
    ...
    def login(self, email: str, password: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self._print_response(
                    "Login selhal", response.status_code, response.json(), success=False
                )
                return False
    ...
    def get_clients(self) -> Optional[list]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self._print_response(
                    "Načtení klientů selhalo", response.status_code, response.json(), success=False
                )
                return None
        except Exception as e:
            print(f"❌ Chyba: {str(e)}")
    ...
    def get_projects(self) -> Optional[list]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self._print_response(
                    "Načtení projektů selhalo", response.status_code, response.json(), success=False
                )
                return None
        except Exception as e:
            print(f"❌ Chyba: {str(e)}")
    ...
    def create_project(
        self, client_id: int, name: str, budget: float, hours: float
    ) -> Optional[dict]:
        ...
        try:
            ...
            if response.status_code == 201:
                data = response.json()
                self._print_response(
                    f"Projekt vytvořen: {name}", response.status_code, data, success=True
                )
                return data
            else:
                self._print_response(
    ...
    def get_project_detail(self, project_id: int) -> Optional[dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                self._print_response(
                    f"Detail projektu {data.get('name')}", response.status_code, data, success=True
                )
                return data
            else:
                self._print_response(
                    "Načtení detailu selhalo", response.status_code, response.json(), success=False
                )
                return None
        except Exception as e:
            print(f"❌ Chyba: {str(e)}")
...
def main():
    ...
    new_project = tester.create_project(
        client_id=client_id, name="Mobile App Development", budget=75000.00, hours=300.00
    )
