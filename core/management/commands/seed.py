...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        clients_data = [
            ...
            {'name': 'Fotogalerie Svoboda', 'email': 'svoboda@example.com', 'phone': '+420 777 234 567'},
        ...
        # Vytvoř projekty
        projects_data = [
            {'name': 'Redesign webu', 'client': 'Web Novák', 'rate': 500, 'status': 'in_progress', 'deadline': timezone.now().date() + timedelta(days=10)},
            {'name': 'Galerie fotek', 'client': 'Fotogalerie Svoboda', 'rate': 400, 'status': 'new', 'deadline': timezone.now().date() + timedelta(days=7)},
            {'name': 'Nový e-shop', 'client': 'E-shop Kolář', 'rate': 600, 'status': 'in_progress', 'deadline': timezone.now().date() - timedelta(days=2)},  # Overdue!
            {'name': 'Opravy CSS', 'client': 'Web Novák', 'rate': 300, 'status': 'done', 'deadline': timezone.now().date() - timedelta(days=5)},
        ]
